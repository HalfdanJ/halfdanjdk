...
<Vimeo videoId="1362832" />

First collaboration with choreographer Tina Tarpgaard and programmer [Ole Kristensen](http://ole.kristensen.name/) under the name [Recoil Performance Group](http://recoil-performance.org) on an interactive dance performance. We were invited to create one of the rooms in a large dance performance called _Labyrinth_ by [Danish Dance Theatre](http://www.danskdanseteater.dk/?language=en). We developed a set of scenes in Processing that created live graphics projected on the floor around the two dancers, and software which tracked the dancers using an infrared camera mounted on the ceiling.
